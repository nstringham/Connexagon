...
export const makeTurn = functions.firestore
  .document("games/{gameId}/moves/{userId}")
  .onCreate((moveSnapshot, context) => {
        ...
        return Promise.all([
          transaction.get(gameRef).then((doc) => {
            ...
            if (
              ...
              context.params.userId === game.uids[game.turn % game.players.length]
            ) {
      ...
      return transaction.get(firestore.doc(`games/${id}`)).then((doc) => {
        ...
        if (data.winner === -1) {
          if (context.auth?.uid && data.uids.includes(context.auth?.uid)) {
            if (data.modified.toMillis() < admin.firestore.Timestamp.now().toMillis() - 6.048e8) {
              return transaction.set(
                doc.ref,
                { turn: data.turn + 1, modified: admin.firestore.Timestamp.now() },
                { merge: true }
              );
            } else {
              throw new functions.https.HttpsError(
                "failed-precondition",
                "turn must be a week old to report it as late"
              );
            }
          } else {
            throw new functions.https.HttpsError(
              "permission-denied",
              "must be in a game to report a turn in it as late"
            );
          }
        } else {
...
export const handleUserDataChange = functions.firestore.document("users/{uid}").onUpdate((change) => {
  ...
  if (nickname && nickname !== change.before.data()?.nickname) {
    return firestore
      .collection("games")
      .where("uids", "array-contains", change.after.id)
      .orderBy("modified", "desc")
      .limit(1000)
      .get()
      .then((snapshot) => {
        return Promise.all(
          snapshot.docs.map((gameDoc) => {
  ...
  const check = (id: number) => {
    ...
    if (board[id].tower && (board[id].owner === -1 || board[id].owner === player)) {
      ...
      if (
        ...
        undefined ===
          gridData
            .getNeighboringHexes(id, sideLength)
            .find((neighborID) => neighborID !== -1 && board[neighborID].owner !== player)
      ) {
...
async function makeGame(size: number, uids: string[]) {
  ...
  const players = await Promise.all(
    shuffledUIDs.map(
      async (uid) =>
        ((await firestore.doc("users/" + uid).get()).data() as UserData | undefined) || { nickname: "[No Name]" }
    )
  );
